    ...
    def get_default_config() -> dict:
        ...
        return {
            ...
            "dict_dir": os.path.join(
                base_dir, "voicevox", "dict", "open_jtalk_dic_utf"
            ),
        }
...
class voicevox:
    ...
    def __init__(self, text: str, style_id: int, pitch: int, speed: float):
        ...
        if self.config["voicevox"]["edition"]["engine"]:
            ...
            self.params = {"text": text, "speaker": style_id}
    ...
    async def init(cls) -> None:
        ...
        if not cls._initialized:
            if cls._synthesizer is None:
                onnxruntime = await Onnxruntime.load_once(
                    filename=cls._instance.voicevox_config["onnxruntime_path"]
                )
                open_jtalk = await OpenJtalk.new(
                    cls._instance.voicevox_config["dict_dir"]
                )
            ...
            model_count = 0
            model_loaded = set()
            for model_file in Path(cls._instance.voicevox_config["vvm_path"]).glob(
                "*.vvm"
            ):
                try:
                    ...
                    logger.error(
                        f"モデル {model_file.name} の読み込みに失敗しました: {e}"
                    )
                    continue
    ...
    async def get_audio(self) -> bytes:
        try:
            if self.config["debug"]:
                logger.debug(
                    f"音声生成を開始 - テキスト: {self.text}, スタイルID: {self.style_id}, 速度: {self.speed}"
                )
            if self.config["voicevox"]["edition"]["core"]:
                if voicevox._synthesizer is None:
                    raise RuntimeError("シンセサイザーが初期化されていません")

                audio_query = await voicevox._synthesizer.create_audio_query(
                    self.text, self.style_id
                )
    ...
    async def _get_engine(self) -> bytes:
        async with aiohttp.ClientSession(self.url) as session:
            ...
            if json_response.status != 200:
                raise Exception(
                    f"audio_queryのリクエストに失敗しました: {json_data['detail'][0]['msg']}"
                )
