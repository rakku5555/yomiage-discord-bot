...
from vc import message_queues, reading_tasks, update_voice_settings
...
def setup_commands(tree: app_commands.CommandTree):
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    ...
                    description=f"{voice_channel.mention}に参加しました！このチャンネルのメッセージを読み上げます。",
                )
            )

            await db.set_read_channel(
                interaction.guild_id, voice_channel.id, interaction.channel_id
            )
        except discord.ClientException:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"すでに{voice_channel.mention}に接続しています。",
                ),
                ephemeral=True,
            )
    ...
    async def leave(interaction: discord.Interaction):
        ...
        try:
            if interaction.guild_id in message_queues:
                ...
                if (
                    interaction.guild_id in reading_tasks
                    and not reading_tasks[interaction.guild_id].done()
                ):
    ...
    autojoin_group = app_commands.Group(
        name="autojoin", description="自動参加機能の設定"
    )

    @autojoin_group.command(
        name="add",
        description="自動参加するボイスチャンネルと読み上げるテキストチャンネルを設定します",
    )
    @app_commands.describe(
        voice="参加するボイスチャンネル", text="読み上げるテキストチャンネル"
    )
    async def autojoin_add(
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"設定の更新に失敗しました: {str(e)}",
                )
            )
    ...
    async def autojoin_remove(
        interaction: discord.Interaction, voice: discord.VoiceChannel
    ):
        await ensure_db_connection()
        ...
        try:
            ...
            if voice.id != autojoin[0]:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        color=discord.Color.red(),
                        description=f"指定されたボイスチャンネル {voice.mention} の自動参加設定はありません。",
                    ),
                    ephemeral=True,
                )
                return
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.green(),
                    description=f"ボイスチャンネル {voice.mention} の自動参加設定を削除しました。",
                )
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"設定の削除に失敗しました: {str(e)}",
                )
            )

    @autojoin_group.command(
        name="list", description="現在の自動参加設定一覧を表示します"
    )
    async def autojoin_list(interaction: discord.Interaction):
        ...
        if not voice_channel or not text_channel:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description="設定されたチャンネルが見つかりません。",
                ),
                ephemeral=True,
            )
            return
    ...
    async def setvoice(
        ...
        if engine.startswith("aquestalk"):
            ...
            if speed < 50 or speed > 200:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        color=discord.Color.red(),
                        description="AquesTalkの速度は50から200の間で指定してください。",
                    ),
                    ephemeral=True,
                )
                return
            ...
            if speed < 0.5 or speed > 5:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        color=discord.Color.red(),
                        description="VOICEVOX/AivisSpeechの速度は0.5から10の間で指定してください。",
                    ),
                    ephemeral=True,
                )
                return
        ...
        match engine:
            case "aquestalk1":
                is_valid, error_message = validate_voice_engine(
                    engine, voice, config, voice_characters
                )
                if not is_valid:
                    await interaction.response.send_message(
                        embed=discord.Embed(
                            color=discord.Color.red(), description=error_message
                        ),
                        ephemeral=True,
                    )
                    return
    ...
    async def skip(interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                color=discord.Color.green(), description="読み上げを停止しました。"
            )
        )
    ...
    async def dict_add(interaction: discord.Interaction, word: str, to: str):
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"単語の登録に失敗しました: {str(e)}",
                )
            )
    ...
    async def dict_list(interaction: discord.Interaction):
        ...
        try:
            ...
            embed, view = create_dict_pagination_response(
                replacements, "登録されている単語一覧"
            )
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"単語一覧の取得に失敗しました: {str(e)}",
                )
            )
        ...
        try:
            ...
            if word not in replacements:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        color=discord.Color.red(),
                        description=f"単語「{word}」は登録されていません。",
                    ),
                    ephemeral=True,
                )
                return
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"単語の削除に失敗しました: {str(e)}",
                )
            )

    tree.add_command(dict_group)

    global_dict_group = app_commands.Group(
        name="global_dict", description="グローバル辞書機能の設定"
    )

    @global_dict_group.command(
        name="add", description="グローバル単語の読み方を登録します"
    )
    @app_commands.describe(word="登録する単語", to="変換後の読み方")
    async def global_dict_add(interaction: discord.Interaction, word: str, to: str):
        word = word.lower()
        to = jaconv.hira2kata(to)
        await ensure_db_connection()
    ...
    @global_dict_group.command(
        name="list", description="グローバル登録されている単語の一覧を表示します"
    )
    async def global_dict_list(interaction: discord.Interaction):
        await ensure_db_connection()
    ...
    async def mute(interaction: discord.Interaction, user: discord.Member):
        ...
        if user.bot:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description="ボットのメッセージは読み上げられません。",
                ),
                ephemeral=True,
            )
            return
        ...
        if await db.is_user_muted(interaction.guild_id, user.id):
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.dark_orange(),
                    description=f"{user.name}はすでにミュートされています。",
                ),
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.green(),
                    description=f"{user.name}のメッセージを読み上げないように設定しました。",
                )
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"ミュート設定に失敗しました: {str(e)}",
                )
            )

    @tree.command(
        name="unmute",
        description="ミュートされたユーザーのメッセージを読み上げるようにします",
    )
    ...
    async def unmute(interaction: discord.Interaction, user: discord.Member):
        ...
        if not await db.is_user_muted(interaction.guild_id, user.id):
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.dark_orange(),
                    description=f"{user.name}はミュートになっていません。",
                ),
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.green(),
                    description=f"{user.name}のメッセージを読み上げるように設定しました。",
                )
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"ミュート解除に失敗しました: {str(e)}",
                )
            )

    @tree.command(
        name="mutelist", description="ミュートされているユーザーの一覧を表示します"
    )
        ...
        try:
            ...
            if not muted_users:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        color=discord.Color.dark_orange(),
                        description="ミュートされているユーザーはいません。",
                    ),
                    ephemeral=True,
                )
                return
            ...
            await interaction.response.send_message(
                embed=discord.Embed(color=discord.Color.purple(), description=message),
                ephemeral=True,
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"ミュートユーザー一覧の取得に失敗しました: {str(e)}",
                )
            )

    dynamic_group = app_commands.Group(
        name="dynamic_join", description="動的自動参加の設定"
    )

    @dynamic_group.command(name="add", description="動的自動参加の設定を追加します")
    @app_commands.describe(text="読み上げるテキストチャンネル")
    async def dynamic_join_add(
        interaction: discord.Interaction, text: discord.TextChannel
    ):
        await ensure_db_connection()
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.green(),
                    description=f"動的自動参加の設定を追加しました。\nテキストチャンネル: {text.mention}",
                )
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"動的自動参加の設定に失敗しました: {str(e)}",
                )
            )

    @dynamic_group.command(name="remove", description="動的自動参加の設定を削除します")
    @app_commands.describe(text="削除するテキストチャンネル")
    async def dynamic_join_remove(
        interaction: discord.Interaction, text: discord.TextChannel
    ):
        await ensure_db_connection()
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.green(),
                    description=f"動的自動参加の設定を削除しました。\nテキストチャンネル: {text.mention}",
                )
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"動的自動参加の設定の削除に失敗しました: {str(e)}",
                )
            )

    @dynamic_group.command(
        name="list", description="動的自動参加の設定一覧を表示します"
    )
    async def dynamic_join_list(interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"動的自動参加の設定一覧の取得に失敗しました: {str(e)}",
                )
            )
        ...
        try:
            ...
            if not read_channel:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        color=discord.Color.red(),
                        description="読み上げチャンネルが設定されていません。先に/joinコマンドでボイスチャンネルに参加してください。",
                    ),
                    ephemeral=True,
                )
                return
            ...
            await db.set_read_channel(
                interaction.guild_id, voice_channel_id, interaction.channel_id
            )
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.green(),
                    description="読み上げるテキストチャンネルを変更しました。",
                )
            )
        except Exception as e:
            await interaction.response.send_message(
                embed=discord.Embed(
                    color=discord.Color.red(),
                    description=f"読み上げるテキストチャンネルの変更に失敗しました: {str(e)}",
                )
            )
...
def create_dict_pagination_response(
    replacements: dict, title: str, color: discord.Color | None = None
...
class PaginationView(View):
    ...
    async def update_embed(self, interaction: discord.Interaction):
        embed = self.embed_factory(
            self.items, self.page, self.page_size, self.total_pages
        )
