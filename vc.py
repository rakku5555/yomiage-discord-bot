...
async def speak_in_voice_channel(
...
async def process_message_queue(guild_id: int):
    while True:
        try:
            ...
            message, engine, voice_name, pitch, speed, voice_client, accent, lmd = (
                message_data
            )

            await speak_in_voice_channel(
                voice_client, engine, message, voice_name, pitch, speed, accent, lmd
            )
            if debug:
...
async def read_message(
    ...
    if not isinstance(message, str):
        ...
        if (
            message.guild.id not in channels
            or message.channel.id != channels[message.guild.id][1]
        ):
            return
    ...
    message = apply_replacements(
        message.lower(), await db.get_dictionary_replacements(guild.id)
    )
    message = apply_replacements(
    ...
    await message_queues[guild.id].put(
        (message, engine, voice_name, pitch, speed, voice_client, accent, lmd)
    )
...
def update_voice_settings(
    ...
    current_voice_settings[(guild_id, user_id)] = (
        engine,
        voice_name,
        pitch,
        speed,
...
async def pitch_convert(audio_data: bytes, pitch: int) -> bytes:
    process = await asyncio.create_subprocess_exec(
        ...
        "pipe:0",
        "-af",
        f"asetrate=8000*{pitch}/100,atempo=100/{pitch}",
        "-ar",
        "8000",
        "-ac",
        "1",
...
def kana_convert(word: str) -> str:
    with warnings.catch_warnings(
        action="error", category=kanalizer.IncompleteConversionWarning
    ):
        try:
